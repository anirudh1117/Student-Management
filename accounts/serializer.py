...
from utils.commonFunction import generate_password,string_to_uuid, convert_date_time_to_unix_time_millis, get_Choices, common_error_message
...
class UserAccountReadSerializer(serializers.ModelSerializer):
    ...
    def get_date_of_birth(self,userAccount):
        if userAccount.date_of_birth is None:
            return ""
        return ""
        #return convert_date_time_to_unix_time_millis(userAccount.date_of_birth)


    class Meta:
        model = UserAccount
        fields = ['id','email','first_name','middle_name','last_name','date_of_birth','address','phoneNo','bloodGroup','role','profileLogo','created_at','updated_at','last_login','gender', 'school']

class UserAccoutWriteSerializer(serializers.ModelSerializer):
    password = serializers.CharField(read_only=True)
    groups = serializers.ListSerializer(child=serializers.CharField(),required=False)
    address = serializers.JSONField(write_only=True)
    role = serializers.ChoiceField(choices=get_Choices(constants.Role_Choices), required=True)
    ...
    def create(self, validated_data):
        ...
        allowed_chars = validated_data['email'] + validated_data.get('date_of_birth','!@#$%')
        ...
        if account.role.lower() == "student":
            print(others)
            studentSerializer = StudentWriteSerializer(data=others, context = { 'request' : request})
        ...
        elif account.role.lower() == "staff":
            print(others)
            staffSerializer = StaffWriteSerializer(data=others, context = { 'request' : request})
            ...
            guardianSerializer = GuardianWriteSerializer(data=others, context = { 'request' : request})
    ...
    def validate(self, attrs):
        ...
        if not superuser:
            if not self.user.school:
                raise serializers.ValidationError(common_error_message("Sorry, You are not linked with any school!"))
            elif not self.user.school.current_session:
                raise serializers.ValidationError(common_error_message("Sorry, Your school is not registered for this Session!"))
